...
public class Sort implements MyJob {
	...
	extends Mapper<Text, Text, DoubleWritable, Text>{
		...
		public void map(Text key, Text value, Context context)
		    ...
		    String[] richAirports = value.toString().split(",");
		    for (String richAirport : richAirports) {
		        String[] result = richAirport.split("-");
		        context.write(new DoubleWritable(Double.parseDouble(result[1])), new Text(key.toString() + "," + result[0]));
            }
		}
	}
