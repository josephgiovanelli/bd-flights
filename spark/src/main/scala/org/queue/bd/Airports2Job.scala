...
object Airports2Job {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sc = new SparkContext(new SparkConf().setAppName("Spark Airports2Job"))

    val rddAirports = sc.textFile("hdfs:/user/jgiovanelli/flights-dataset/clean/airports")
      .map(x => new Airport(x))
      .map(x => (x.getIata_code, x.getAirport))
      .flatMap(x => TimeSlot.values().toStream.map(y => ((x._1, y),x._2)))
    ...
    val rddResult = rddFlights
      .join(rddAirports)
      .map({ case (k, v) => ((v._2, k._2), v._1._1 / v._1._2 )})
